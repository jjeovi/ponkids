...
//            SimpleDateFormat dateFormatParser = new SimpleDateFormat("yyyy/MM/dd"); //검증할 날짜 포맷 설정
     ...
     * @throws ParseException
     */
    public static LocalDateTime strToLDT( String requestedAt ) {
        // 'T' 포함 여부에 따라 다른 포맷 적용
        DateTimeFormatter formatter;
        if ( requestedAt.contains( "T" ) ) {
            formatter = DateTimeFormatter.ISO_OFFSET_DATE_TIME; // T 포함 형식
        } else {
            formatter = DateTimeFormatter.ofPattern( "yyyy-MM-dd HH:mm:ssXXX" ); // 공백 사용 형식
        }
