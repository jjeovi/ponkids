...
@SequenceGenerator(	// TODO SEQUENCE setting
        name = "SEQ_TB_LCTRE_REQST_DETAIL_SN",
        sequenceName = "SEQ_TB_LCTRE_REQST_DETAIL_SN",
        initialValue = 1,
        allocationSize = 1
)
@Where( clause = "del_yn = 'N'") // DEFAULT 로 WHERE DEL_YN = 'N' 문을 추가하여 조회
@SQLDelete(sql = "UPDATE {h-schema}tb_lctre_reqst_detail SET del_yn ='Y', updt_dt = now() WHERE lctre_reqst_detail_sn = ?") // delelte 시 실행 (ex ) ~Repository.deleteById)
@Table( name = "tb_lctre_reqst_detail" )
public class LctreReqstDetail extends BaseTimeEntity {
	
	@Id
	@GeneratedValue( strategy = GenerationType.SEQUENCE, generator = "SEQ_TB_LCTRE_REQST_DETAIL_SN" )
