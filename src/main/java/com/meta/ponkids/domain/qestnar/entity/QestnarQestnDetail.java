...
@SequenceGenerator(	
        name = "SEQ_TB_qestnar_qestn_detail_SN",
        sequenceName = "SEQ_TB_qestnar_qestn_detail_SN",
        initialValue = 1,
        allocationSize = 1
)
@Where( clause = "del_yn = 'N'") // DEFAULT 로 WHERE DEL_YN = 'N' 문을 추가하여 조회
@SQLDelete(sql = "UPDATE {h-schema}tb_qestnar_qestn_detail SET del_yn ='Y', updt_dt = now() WHERE qestnar_qestn_detail_sn = ?") // delelte 시 실행 (ex ) ~Repository.deleteById)
@Table( name = "TB_qestnar_qestn_detail" )
public class QestnarQestnDetail extends BaseTimeEntity {
	
	@Id
	@GeneratedValue( strategy = GenerationType.SEQUENCE, generator = "SEQ_TB_qestnar_qestn_detail_SN" )
