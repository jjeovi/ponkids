...
@SequenceGenerator(
        name = "SEQ_TB_qestnar_qestn_SN",
        sequenceName = "SEQ_TB_qestnar_qestn_SN",
        initialValue = 1,
        allocationSize = 1
)
@Where( clause = "del_yn = 'N'") // DEFAULT 로 WHERE DEL_YN = 'N' 문을 추가하여 조회
@SQLDelete(sql = "UPDATE {h-schema}tb_qestnar_qestn SET del_yn ='Y', updt_dt = now() WHERE qestnar_qestn_sn = ?") // delelte 시 실행 (ex ) ~Repository.deleteById)
@Table( name = "TB_qestnar_qestn" )
public class QestnarQestn extends BaseTimeEntity {
	
	@Id
	@GeneratedValue( strategy = GenerationType.SEQUENCE, generator = "SEQ_TB_qestnar_qestn_SN" )
