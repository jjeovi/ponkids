...
import com.meta.ponkids.domain.payment.refund.service.RefundService;
...
import com.meta.ponkids.global.util.generator.IdempotencyKeyGenerator;
...
//		List<LctreReqstListDto> targetLctreReqsts = lctreReqstService.getListByClassReqstSn(pk);
    ...
    public String reqstHistoryCancel( @RequestParam( required = true ) Long classReqstSn,    // 타입 체크
                                      HttpServletRequest request,
                                      Model model ) throws Exception {
        
        LocalDateTime cancelRequestTime = LocalDateTime.now();  // 취소 요청일시
        ...
        // 0. 유효성 체크 작업
        // 1. 결제 정보 취소
        // 2. 클래스 신청 (TB_CLASS_REQST) 에서 삭제  (1 건)
        // 3. 수업 신청 ( TB_LCTRE_REQST) 에서 삭제  ( 여러건 가능 )
        // 4. 수업 신청 상세 ( TB_LCTRE_REQST_DETAIL ) 에서 삭제 ( 2번 count 에서 추가로 여러건 또 가능 )
        // 5. 삭제 이후 프로세스 수행 : 삭제 한 신청건의 수업들( lctreReqsts ) 을 순회해 각 수업의 모집인원 여부와 예비모집인원여부를 체크
        //    5-1. 모집인원여부, 모집인원여부 Y, 예비인원여부 N 일 경우 :
        //         5-1-1. (실시간 신청 인원 수 + 해당수업의삭제한 예비인원N인 건 수) 와 모집 인원 수가 같다면
        //                - 예비인원 중 현재신청건이 아니면서 삭제되지않고, 가장 먼저 등록한 1명 선택
        //                - 예비인원여부를 N으로 설정
        //                - 수업대상자 전환 안내 메일 발송
        // + 결제 정보 취소
        // 결제 정보는 기간에 따라 취소 로직을 구현한다.
        // =================================================================================
        ...
        // 0. 유효성 체크 작업
        boolean validationResult = classPaymentValidator.validateCancel( classReqstSn, model, BASIC_PATH, cancelRequestTime );
        if ( !validationResult ) {
            return "common/alert";
        }
        ...
        RefundContext refundContext = RefundContext.builder().
                ...
                classPayment( classPaymentRepository.findByClassReqst_ClassReqstSn( classReqstSn ).orElse( null ) ).build();
        
        refundContext = refundService.calculateRefund( refundContext );
        
        
        // 기존 멱등키 존재하는 지 확인
        String existIdempotencyKey = refundContext.getClassPayment().getIdempotencyKey();
        
        // 멱등키가 없거나
        // 멱등키가 만료되었다면
        if ( !StringUtils.hasText( existIdempotencyKey ) ||
                ( StringUtils.hasText( existIdempotencyKey ) ||  isIdempotencyKeyExpired( refundContext.getClassPayment() ) )
        ) {
            // 멱등키 생성
            String idempotencyKey = IdempotencyKeyGenerator.generatorIdempotencyKey();
            LocalDateTime idempotencyKeyCreatedAt = LocalDateTime.now();
            refundContext.getClassPayment().setIdempotencyKey( idempotencyKey );
            refundContext.getClassPayment().setIdempotencyCreatedAt( idempotencyKeyCreatedAt );
        }
        
        
        // 결제 정보 취소
        ResponseEntity<JSONObject> result =  refundService.paymentCancel( refundContext, request );
        
        if ( result == null ) {
            model.addAttribute( "resultMsg", "취소 요청 중 문제가 발생하였습니다. 다시 시도해주세요." );
        ...
        // 결제 정보 취소
        // 결제 정책에 의한 금액 산정
        // List<LctreReqstListDto> lctreReqsts
        // classReqstSn
        // 취소요청시각
        ...
        // 새로운 list 선언 ( long 변수)
        // 결국 rltmDelReqstList 에서는 해당 수업의 예비인원 여부가 N인 삭제자의 수를 찾기 위함
        ...
        // 2. 클래스 신청 (TB_CLASS_REQST) 에서 삭제  (1 건)
        classReqstService.deleteById( classReqstSn );
        
        // 3. 수업 신청 ( TB_LCTRE_REQST) 에서 삭제  ( 여러건 가능 )
        // 4. 수업 신청 상세 ( TB_LCTRE_REQST_DETAIL ) 에서 삭제 ( 2번 count 에서 추가로 여러건 또 가능 )
        
        // 3,4 번 동시에 수행.
        lctreReqstService.deleteByClassReqstSn( classReqstSn );
        
        // 삭제 처리 이후 프로세스
        // 5. 삭제 이후 프로세스 수행 : 삭제 한 신청건의 수업들( lctreReqsts ) 을 순회해 각 수업의 모집인원 여부와 예비모집인원여부를 체크
        //    5-1. 모집인원여부, 예비모집인원여부가 Y, (신청자기준)예비인원여부 N 일 경우 :
        //         5-1-1. (실시간 신청 인원 수 + 해당수업의삭제한 예비인원N인 건 수) 와 모집 인원 수가 같다면 : 예비인원 중 현재신청건이 아니면서 삭제되지않고, 가장 먼저 등록한 1명의 예비인원여부를 N으로 설정
        ...
        for ( LctreReqstListDto lctreReqst : lctreReqsts ) {                                                        // 4. 삭제 이후 프로세스 수행 : 삭제 한 신청건의 수업들( lctreReqsts ) 을 순회해 각 수업의 모집인원 여부와 예비모집인원여부를 체크
            ...
            if ( "Y".equals( rcritNmprSetYn ) &&
                    "Y".equals( preparRcritNmprSetYn ) &&
                    "N".equals( preparNmprYn ) ) {                                                                        //    5-1. 모집인원여부, 예비모집인원여부가 Y, (신청자기준)예비인원여부 N 일 경우 :
                
                if ( rltmReqstNmprCo != null && nowDelSameLctreCo != null && rcritNmprCo != null && rcritNmprCo.equals( ( rltmReqstNmprCo + nowDelSameLctreCo ) ) ) {    //         5-1-1. (실시간 신청 인원 수 + 해당수업의삭제한 예비인원N인 건 수) 와 모집 인원 수가 같다면
                    // 예비인원 중 현재신청건이 아니면서 삭제되지않고, 가장 먼저 등록한 1명 선택
                    // 예비인원여부를 N으로 설정
                    // 수업대상자 선정 알림 메일 발송
        ...
        // 메시지 출력 및 url 이동 처리
        model.addAttribute( "resultMsg", "정상적으로 취소 처리되었습니다." );
        model.addAttribute( "moveUrl", moveUrl );
    ...
    public boolean isIdempotencyKeyExpired( ClassPayment classPayment ) {
        // 멱등키 생성 시간
        LocalDateTime createdAt = classPayment.getIdempotencyCreatedAt();
        
        if ( createdAt == null ) {
            return false; // 멱등키 생성 시간이 없으면 만료되지 않음
        }
        
        // 현재 시간
        LocalDateTime now = LocalDateTime.now();
        
        // 멱등키 생성 시간이 15일 이상 경과했는지 확인
        long daysBetween = ChronoUnit.DAYS.between( createdAt, now );
        
        return daysBetween > 15; // 15일 이상 경과하면 true, 아니면 false
    }
