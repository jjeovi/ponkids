...
@SequenceGenerator(	
        name = "SEQ_TB_class_review_SN",
        sequenceName = "SEQ_TB_class_review_SN",
        initialValue = 1,
        allocationSize = 1
)
@Where( clause = "del_yn = 'N'") // DEFAULT 로 WHERE DEL_YN = 'N' 문을 추가하여 조회
@SQLDelete(sql = "UPDATE {h-schema}tb_class_review SET del_yn ='Y', updt_dt = now() WHERE class_review_sn = ?") // delelte 시 실행 (ex ) ~Repository.deleteById)
@Table( name = "tb_class_review" )
public class ClassReview extends BaseTimeEntity {
	
	@Id
	@GeneratedValue( strategy = GenerationType.SEQUENCE, generator = "SEQ_TB_class_review_SN" )
