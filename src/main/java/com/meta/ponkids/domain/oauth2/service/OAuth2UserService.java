...
//			 1-1. 같은 이메일로 가입된 계정(A)이 있을 경우
            ...
            loginDto     = loginRepository.getLogin( userId );
            loginDto.setAttributes( oAuth2User.getAttributes() );
...
//			-> 해당 SNS 로 연동이 되어있는지 확인
            switch ( snsType ) {
                ...
                case "google":
                    ...
                    if ( StringUtils.hasText( user.getSnsGoogleCntnYn() ) ) snsCntnYn = true;
                    break;
                
                case "kakao":
                    // 연계 sns 카카오 에 값이 있다면, 연계여부 는 Y 로 setting
                    if ( StringUtils.hasText( user.getSnsKakaoCntnYn() ) ) snsCntnYn = true;
                    break;
